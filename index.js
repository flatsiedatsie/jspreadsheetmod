...
var formula = (function() {
    ...
    var secureFormula = function(oldValue, runtime) {
        console.log("secureFormula: oldValue: ", oldValue);
        var newValue = '';
        var inside = 0;
        ...
        console.log("secureFormula: newValue: ", newValue);
        return newValue;
    }
    ...
    var F = function(expression, variables, i, j, obj) {
        console.error("F: ", expression, variables, i, j);
        // Global helpers
        instance = obj;
        x = i
        y = j;
        ...
        console.log("function s: ", s, ", expression: ", expression);
        // Calculate
...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            onblur:null,
            onstartrowdrag: null,
            onstartcolumndrag: null,
            onrowdragging: null,
            oncolumndragging: null,
            onendrowdrag: null,
            onendcolumndrag: null,
            onclearcells:null,
            onchangeheader:null,
            oncreateeditor:null,
            oneditionstart:null,
        ...
        obj.dispatch = function(event) {
            //console.log("obj.dispatch: event: ", event);
            // Dispatch events
            if (! obj.ignoreEvents) {
                // Call global event
                if (typeof(obj.options.onevent) == 'function') {
                    //console.log("obj.dispatch -> global event. this, arguments: ", this, arguments);
                    var ret = obj.options.onevent.apply(this, arguments);
                }
                // Call specific events
                if (typeof(obj.options[event]) == 'function') {
                    //console.log("obj.dispatch -> specific event. this, arguments: ", this, arguments);
        ...
        obj.prepareTable = function() {
            console.log("jsp: in prepareTable. columns and data: ", obj.options.columns, obj.options.data);
            ...
            for (var i = 0; i < size; i++) {
                ...
                if (obj.options.columns[i].type == 'autocomplete' || obj.options.columns[i].type == 'dropdown') {
                    console.log("jsp: Pre-load initial source for json autocomplete");
                    // if remote content
                    if (obj.options.columns[i].url) {
                        console.log("jsp: spotted URL: ", obj.options.columns[i].url);
                        multiple.push({
            ...
            console.log("jsp: in prepareTable. multiple.length: ", multiple.length);
        ...
        obj.createTable = function() {
            console.log("jsp: in createTable. columns and data: ", obj.options.columns, obj.options.data);
            // Elements
            ...
            obj.searchInput.setAttribute("type", 'search');
            ...
            filename_div.classList.add('filename-input-container');
            let filename_input_el = document.createElement("input");
            filename_input_el.classList.add('filename-input');
            filename_input_el.placeholder = 'Filename';
            filename_input_el.onfocus = () => {
                obj.resetSelection();
            }
            filename_div.appendChild(filename_input_el);
            filter.appendChild(filename_div);
            */
            ...
            ads.setAttribute('href', 'https://tijmenschep.com');
        ...
        */
        ...
        obj.createCell = function(i, j, value) {
            ...
            if (obj.options.columns[i].editor) {
                ...
                if (obj.options.columns[i].type == 'hidden') {
                    ...
                    if (value && typeof value == 'string' && value.substr(0, 10) == 'data:image') {
                        var img = document.createElement('img');
                        img.src = value;
                        td.appendChild(img);
                    }
                    ...
                    if (toolbar[i].onclick && typeof(toolbar[i].onclick)) {
                        ...
                        toolbarItem.onclick = function() {
                            ...
                            if(obj.highlighted.length == 0){
                                console.error("no cells highlighted! Could try to recover fresh-highlight");
                                obj.highlighted = document.querySelectorAll('.fresh-highlighted');
                            }
        ...
        obj.openEditor = function(cell, empty, e) {
            //console.log("jexcel: in openEditor");
            ...
            if (cell.classList.contains('readonly') == true) {
                ...
                //console.log("obj.edition: ", obj.edition);
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'hidden') {
                        ...
                        //console.log("jexcel: editor: dropdown or autocomplete");
                        ...
                        console.log("jexcel: basic editor: options.data[y][x])", x,y,obj.options.data[y][x],obj);
                        // Value
                        var value = empty == true ? '' : obj.options.data[y][x];
                        console.log("jexcel: basic editor: value: ", typeof value, value);
                        
                        if(typeof value == 'string' && value.startsWith('data:image')){
                            var img = document.createElement('img');
                            img.src = value; //cell.children[0];
                            // Create editor
                            var editor = createEditor('div');
                            editor.style.position = 'relative';
                            var div = document.createElement('div');
        ...
        obj.closeEditor = function(cell, save) {
            console.log("jexcel: in closeEditor. save, cell: ", save, cell);
            ...
            if (save == true) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if(typeof cell.children[0] == 'undefined'){
                        console.error("closeEditor: cell.children[0] was undefined");
                    } else{
                        // Native functions
        ...
        obj.getValue = function(cell, processedValue) {
            console.log("jexcel: in getValue");
        ...
        obj.setValue = function(cell, value, force) {
            console.log("in jexcel setValue: value, force: ", value, force);
            var records = [];
        ...
        obj.updateTableReferences = function() {
            console.log("in updateTableReferences");
            // Update headers
            for (var i = 0; i < obj.headers.length; i++) {
            ...
            console.error("affectedTokens: ", affectedTokens);
            // Update formulas
        ...
        obj.updateFormulas = function(referencesToUpdate) {
            console.log("in jexcel: updateFormulas. referencesToUpdate: ", referencesToUpdate);
            // Update formulas
            for (var j = 0; j < obj.options.data.length; j++) {
        ...
        obj.updateFormula = function(formula, referencesToUpdate) {
            console.log("in jexcel: UPDATEFormula. formula, referencesToUpdate: ", formula, referencesToUpdate);
            var testLetter = /[A-Z]/;
            var testNumber = /[0-9]/;
        ...
        var secureFormula = function(oldValue) {
            console.log("in jexcel secureFormula. oldValue: ", oldValue);
            var newValue = '';
            var inside = 0;
            ...
            console.log("in jexcel secureFormula. newValue: ", newValue);
            return newValue;
        }
        ...
        obj.executeFormula = function(expression, x, y) {
            console.log("in jexcel: EXECUTEFormula. expression, x, y: ", expression, x, y);
            var formulaResults = [];
            ...
            var execute = function(expression, x, y) {
             // Parent column identification
                ...
                var tokensUpdate = function(tokens) {
                    console.log("in jexcel tokensUpdate: tokens: ", tokens);
                    for (var index = 0; index < tokens.length; index++) {
                        var f = [];
                ...
                console.log("jexcel: tokens: ", tokens);
                ...
                if (tokens && tokens.indexOf(parentId) > -1) {
                    ...
                    try {
                        ...
                        console.log("formula as javascript: ", res);
                    } catch (e) {
                        var res = '#ERROR';
                        console.log(e)
                    }
            ...
            console.log("going to execute: expression, x, y: ", expression, x, y);
        ...
        obj.parseNumber = function(value, columnNumber) {
            console.log("jexcel: in parseNumer. value: ", value);
            // Decimal point
         ...
         */
        /*
        obj.download_save_hack = function(includeHeaders,uuid) {
        ...
        */
         ...
         * @return null
         */
        obj.download_csv = function(includeHeaders) {
            if (obj.options.allowExport == false) {
                console.error('Export not allowed');
            } else {
                // Data
                    ...
                    if (x) {
                        ...
                        items.push({
                            ...
                            onclick:function() {
                                ...
                                show_format(parseInt(obj.selectedCell[0]), parseInt(obj.selectedCell[1]), parseInt(obj.selectedCell[2]), parseInt(obj.selectedCell[3]));
                                //console.log("obj.selectedCell[2] + 1: ", parseInt(obj.selectedCell[2]) + 1);
                                //console.log(typeof obj.selectedCell[2]);
                                /*
                                for(let cx = parseInt(obj.selectedCell[0]); cx < parseInt(obj.selectedCell[2]) + 1;cx++){
                                    for(let cy = parseInt(obj.selectedCell[1]); cy < parseInt(obj.selectedCell[3]) + 1;cy++){
                                        console.log("clearing cells: ", cx, cy);
                                        obj.updateCell(cx, cy, '', true);
                                    }
                                }
                                */
                        ...
                        if(parseInt(obj.selectedCell[0]) < parseInt(obj.selectedCell[2]) || parseInt(obj.selectedCell[1]) < parseInt(obj.selectedCell[3])){
                            ...
                            items.push({
                                ...
                                onclick:function() {
                                    ...
                                    jexcel.current.dispatch('onclearcells', parseInt(obj.selectedCell[0]), parseInt(obj.selectedCell[1]), parseInt(obj.selectedCell[2]), parseInt(obj.selectedCell[3]));
                            ...
                            items.push({
                                title: 'Merge',
                                onclick:function() {
                                    obj.setMerge(jexcel.getColumnNameFromId([ obj.selectedCell[0], obj.selectedCell[1] ]), 1+(obj.selectedCell[2]-obj.selectedCell[0]), 1+(obj.selectedCell[3]-obj.selectedCell[1]));
                                }
                            });
                            ...
                            const cell_name = jexcel.getColumnNameFromId([ x, y ]);
                            if(obj.options.mergeCells[cell_name]){
                                items.push({
                                    title: 'Unmerge',
                                    onclick:function() {
                                        obj.removeMerge(cell_name);
                                    }
                                });
                            }
                        }
        ...
        obj.updateFreezePosition = function() {
            ...
            if (scrollLeft > 50) {
                ...
                const filter_tds = document.querySelectorAll('th:nth-child(2).jexcel_column_filter');
                //console.log("filter_tds: ",filter_tds);
                
                for (var x = 0; x < filter_tds.length; x++) {
                    filter_tds[x].classList.add('jexcel_freezed');
                    filter_tds[x].style.left = width + 'px';
                }
                ...
                const filter_tds = document.querySelectorAll('th:nth-child(2).jexcel_column_filter');
                for (var x = 0; x < filter_tds.length; x++) {
                    filter_tds[x].classList.remove('jexcel_freezed');
                    filter_tds[x].style.left = '';
                }
                for (var i = 0; i < obj.options.freezeColumns; i++) {
                    obj.headers[i].classList.remove('jexcel_freezed');
                    obj.headers[i].style.left = '';
                    
                    //filter_tds[i+1].classList.remove('jexcel_freezed');
                    //filter_tds[i+1].style.left = '';
                    for (var j = 0; j < obj.rows.length; j++) {
                        if (obj.records[j][i]) {
    ...
    jexcel.keyDownControls = function(e) {
        console.log("in jexcel.keyDownControls. e: ", e);
        if (jexcel.current) {
            if (jexcel.current.edition) {
                if (e.which == 27) {
                    ...
                    // Up & Down arrow
                    if(jexcel.current.edition[0].children[0].nodeName == 'INPUT'){
                        if (jexcel.current.options.columns[jexcel.current.edition[2]].type == 'calendar') {
                            jexcel.current.closeEditor(jexcel.current.edition[0], true);
                        } else {
                            jexcel.current.edition[0].children[0].blur();
                            ...
                            if (jexcel.current.options.editable == true) {
                                ...
                                if (jexcel.current.options.columns[columnId].type != 'readonly') {
                                    ...
                                    if (e.keyCode == 32) {
                                        ...
                                        //console.log("backspace, starting editor?");
                                        
                                        // hacked: remove image on backspace
                    ...
                    if (columnId) {
                        ...
                        if (jexcel.current.options.columnResize == true && info.width - e.offsetX < 6) {
                            ...
                            if (jexcel.current.isColMerged(columnId).length) {
                                ...
                                jexcel.current.dispatch('onstartcolumndrag',e.target,columnId);
                    ...
                    if (e.target.classList.contains('jexcel_row')) {
                        ...
                        if (jexcel.current.options.rowResize == true && info.height - e.offsetY < 6) {
                            ...
                            if (jexcel.current.isRowMerged(rowId).length) {
                                ...
                                jexcel.current.dispatch('onstartrowdrag',e.target.parentNode,rowId);
                            }
                        } else {
                        ...
                        if (e.target.classList.contains('jclose') && e.target.clientWidth - e.offsetX < 50 && e.offsetY < 50) {
                            ...
                            if(e.target.getAttribute('data-x') != null){
        ...
        if (jexcel.current) {
            ...
            if (jexcel.current.resizing) {
                ...
                if (jexcel.current.dragging) {
                    //drag_image_el.style.display = 'none';
                    if (jexcel.current.dragging.column) {
                        jexcel.current.dispatch('onendcolumndrag');
                        // Target
                        var columnId = e.target.getAttribute('data-x');
                        // Remove move style
                        ...
                        jexcel.current.dispatch('onendrowdrag');
                ...
                //console.log("jexcel: mouseup. obj.selection & highlighted: ", jexcel.current.selection, jexcel.current.highlighted);
                on_selection_final(jexcel.current.highlighted);
    ...
    jexcel.mouseMoveControls = function(e) {
        ...
        //console.log("mouseMoveControls: e,isMouseAction: ",e, jexcel.isMouseAction);
        ...
        if (jexcel.current) {
            if (jexcel.isMouseAction == true) {
                ...
                if (jexcel.current.resizing) {
                    ...
                    // Hack to get a drag image
                    //drag_image_el.style.display = 'flex';
                    //drag_image_el.style.transform = 'translateY('+(e.clientY-(drag_image_el.clientHeight/2))+'px)';
                    //drag_image_el.style.transform += 'translateX('+(e.clientX-(drag_image_el.clientWidth/2))+'px)';   
                    
                    
                    if (jexcel.current.dragging.column) {
                        jexcel.current.dispatch('oncolumndragging', e);
                        var columnId = e.target.getAttribute('data-x');
                        if (columnId) {
                        ...
                        jexcel.current.dispatch('onrowdragging', e);
                        var rowId = e.target.getAttribute('data-y');
                        if (rowId) {
                            if (jexcel.current.isRowMerged(rowId).length) {
                                console.error('Jspreadsheet: This row is part of a merged cell.');
    ...
    jexcel.tabs = function(tabs, result) {
        ...
        if (! tabs.classList.contains('jexcel_tabs')) {
            ...
            var div = document.createElement('div');
            div.id = 'jexcel-tab-content';
            var content = tabs.appendChild(div);
            
            var div = document.createElement('div');
            div.id = 'jexcel-tab-container';
            var headers = tabs.appendChild(div);
            
        } else {
            var headers = document.getElementByID('jexcel-tab-container'); //tabs.children[1];
            var content = document.getElementByID('jexcel-tab-content'); //tabs.children[0];
        }
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            spreadsheet[i].id = 'jexcel_tab_' + i;
        ...
        content.children[headers.children.length - 1].style.display = 'flex';
