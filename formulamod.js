...
(function(global, factory) {
  ...
  function fillMatrix(matrix, fill_value) {
    ...
    return matrix.map((function(el) {
      return [].concat(_toConsumableArray(el), _toConsumableArray(Array(longestArrayLength - el.length).fill(fill_value ? fill_value : 0)));
    }));
  }
  function flatten() {
    var result;
  ...
  function COUNTIFS() {
    ...
    for (var _i2 = 0; _i2 < args.length; _i2 += 2) {
      var range = flatten(args[_i2]);
      var criteria = args[_i2 + 1];
      var isWildcard = criteria === void 0 || criteria === "*";
      if (!isWildcard) {
        var tokenizedCriteria = parse(criteria + "");
  ...
  function SUMIFS() {
    ...
    var result = 0;
    for (var _i4 = 0; _i4 < range.length; _i4++) {
      var isMeetCondition = false;
      for (var j = 0; j < criteriaLength; j++) {
        var valueToTest = criterias[j * 2][_i4];
  ...
  var bessel = getDefaultExportFromCjs(bessel$1);
  function ASC() {
  ...
  function TEXT(value$1, format_text) {
    ...
    var decimalPlaces = format_text.split(".")[1].match(/0/g).length;
